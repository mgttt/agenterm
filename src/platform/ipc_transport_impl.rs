...
mod windows_pipe {
            ...
            match connect_pipe_instance(&pipe, timeout, &self.endpoint) {
                Ok(()) => {
                    ...
                    Ok(IpcStream {
                        ...
                        inner: StreamInner::LegacyNamedPipe(NamedPipeStream {
                            handle: pipe,
                            timeout,
                        }),
                    })
        ...
        pub(super) fn connect(
            ...
            loop {
                ...
                if handle != INVALID_HANDLE_VALUE {
                    ...
                    return Ok(IpcStream {
                        endpoint: endpoint.clone(),
                        inner: StreamInner::LegacyNamedPipe(Self { handle, timeout }),
                    });
                }
                let error = io::Error::last_os_error();
    ...
    impl PipeSecurity {
        fn for_current_user(endpoint: &IpcEndpoint) -> TransportResult<Self> {
            let sid = crate::platform::ipc::trusted_user_identity()
                .map_err(|error| transport_io(endpoint, error))?
                .bytes;
