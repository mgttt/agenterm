...
fn run() -> anyhow::Result<u8> {
    ...
    match arguments.as_slice() {
        ...
        [mode, ..] if mode == "check-many" => {
            anyhow::bail!("{}", agenterm::script_rh_cli::check_many_requires_rh_error())
        }
    ...
    if invocation.operation == ScriptOperation::Check {
        ...
        if let Some(project_root) = invocation.project_root.as_deref() {
            ...
            for module_source in module_sources {
                agenterm::script_api_validate::validate_available_apis(&module_source).map_err(
                    |failure| {
        ...
        agenterm::script_api_validate::validate_available_apis(&invocation.source).map_err(
            |failure| {
                classify_compile_error(format!("{}: {}", failure.code, failure.message))
            },
        )?;
