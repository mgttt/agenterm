...
const FIXTURES: [(&str, &[&str], &[&str]); 27] = [
    ...
    ("rh_runtime_atomic_write.rh", &["rh_atomic_write("], &[]),
    ("rh_crypto_sha256_file.rh", &["rh_sha256_file("], &[]),
    (
        "rh_bytes_from_array.rh",
        &[
            "rh_bytes_from_array(&[])",
            "rh_bytes_from_array(&[0, 65, 255])",
        ],
        &[],
    ),
    (
    ...
    (
        ...
        &[
            "rh_command_stdin_text(&mut worker",
            "String::from(\"hello\\n\")",
        ],
    ...
    (
        ...
        &[
            "rh_command_args(&mut command",
            "rh_json_string_argv(&arguments)",
        ],
    ...
    ("rh_hash_fnv1a64.rh", &["rh_hash_fnv1a64("], &[]),
    (
        "rh_env_has_get.rh",
        &["rh_env_has(", "rh_env_get("],
        &[
            "rh_host_eval_int(\"std::env::has",
            "rh_host_eval_int(\"std::env::get",
        ],
    ),
    (
...
fn assert_native_only(name: &str, needles: &[&str], anti_needles: &[&str]) {
...
#[test]
fn codegen_revision_is_eighty_five() {
...
fn empty_map_fn_return_emits_native_json_not_set() {
    assert_native_only(FIXTURES[0].0, FIXTURES[0].1, FIXTURES[0].2);
}

#[test]
fn json_map_key_chain_via_locals_emits_native_path_key_writes() {
    assert_native_only(FIXTURES[1].0, FIXTURES[1].1, FIXTURES[1].2);
}

#[test]
fn array_index_property_via_local_emits_index_not_key_path() {
    assert_native_only(FIXTURES[2].0, FIXTURES[2].1, FIXTURES[2].2);
}

#[test]
fn null_unit_compare_emits_native_json_null_check() {
    assert_native_only(FIXTURES[3].0, FIXTURES[3].1, FIXTURES[3].2);
}

#[test]
fn snapshot_tabs_index_return_emits_path_index_not_string_index() {
    assert_native_only(FIXTURES[4].0, FIXTURES[4].1, FIXTURES[4].2);
}

#[test]
fn working_context_dot_chain_via_locals_stays_native_without_extra_host_eval() {
    assert_native_only(FIXTURES[5].0, FIXTURES[5].1, FIXTURES[5].2);
...
fn tab_active_map_key_vs_dot_emits_native_key_and_field_reads() {
    assert_native_only(FIXTURES[6].0, FIXTURES[6].1, FIXTURES[6].2);
}

#[test]
fn snapshot_tabs_map_key_active_emits_array_get_and_path_key_not_string_index() {
    assert_native_only(FIXTURES[7].0, FIXTURES[7].1, FIXTURES[7].2);
}

#[test]
fn gui_window_control_visible_click_emits_native_gui_host_calls() {
    assert_native_only(FIXTURES[8].0, FIXTURES[8].1, FIXTURES[8].2);
...
fn clipboard_get_set_text_emits_native_host_json_calls() {
    assert_native_only(FIXTURES[9].0, FIXTURES[9].1, FIXTURES[9].2);
...
fn rh_host_api_json_task_emits_native_json_and_sleep() {
    assert_native_only(FIXTURES[10].0, FIXTURES[10].1, FIXTURES[10].2);
...
fn runtime_atomic_write_emits_native_host_call() {
    assert_native_only(FIXTURES[11].0, FIXTURES[11].1, FIXTURES[11].2);
...
fn crypto_sha256_file_emits_native_host_call() {
    assert_native_only(FIXTURES[12].0, FIXTURES[12].1, FIXTURES[12].2);
...
fn bytes_from_array_emits_native_bytes_constructors() {
    assert_native_only(FIXTURES[13].0, FIXTURES[13].1, FIXTURES[13].2);
...
fn process_kill_emits_native_host_call() {
    assert_native_only(FIXTURES[14].0, FIXTURES[14].1, FIXTURES[14].2);
...
fn command_arg_emits_native_single_arg_append() {
    assert_native_only(FIXTURES[15].0, FIXTURES[15].1, FIXTURES[15].2);
...
fn string_index_of_emits_native_find_helper() {
    assert_native_only(FIXTURES[16].0, FIXTURES[16].1, FIXTURES[16].2);
...
fn std_fs_write_emits_native_host_call() {
    assert_native_only(FIXTURES[17].0, FIXTURES[17].1, FIXTURES[17].2);
...
fn json_marker_run_properties_emit_native_path_reads() {
    assert_native_only(FIXTURES[18].0, FIXTURES[18].1, FIXTURES[18].2);
...
fn bytes_append_emits_native_bytes_mutation() {
    assert_native_only(FIXTURES[19].0, FIXTURES[19].1, FIXTURES[19].2);
...
fn command_stdin_text_emits_native_command_mutation() {
    assert_native_only(FIXTURES[20].0, FIXTURES[20].1, FIXTURES[20].2);
...
fn child_stdout_emits_native_stream_access() {
    assert_native_only(FIXTURES[21].0, FIXTURES[21].1, FIXTURES[21].2);
...
fn command_args_json_emits_native_argv_coercion() {
    assert_native_only(FIXTURES[22].0, FIXTURES[22].1, FIXTURES[22].2);
...
fn json_stringify_pretty_emits_native_host_call() {
    assert_native_only(FIXTURES[23].0, FIXTURES[23].1, FIXTURES[23].2);
...
fn hash_fnv1a64_emits_native_host_call() {
    assert_native_only(FIXTURES[24].0, FIXTURES[24].1, FIXTURES[24].2);
...
fn env_has_get_emits_native_host_calls() {
    assert_native_only(FIXTURES[25].0, FIXTURES[25].1, FIXTURES[25].2);
...
fn system_time_rfc3339_emits_native_host_call() {
    assert_native_only(FIXTURES[26].0, FIXTURES[26].1, FIXTURES[26].2);
