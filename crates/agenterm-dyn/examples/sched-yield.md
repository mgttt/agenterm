...
Linux uses `libc.so.6`; macOS uses `libSystem.B.dylib`. The probe deliberately
declares a `void` result so completion maps to the language's nil value.
...
(do
  ...
  (if (= attempts 1)
    (dlcall "libc.so.6" "sched_yield" "void")
    0))
```

Successful evaluation returns nil after yielding once. The script does not
