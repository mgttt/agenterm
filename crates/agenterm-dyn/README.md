...
|------|-------------|------------|------------|-----------------|----------------------------|
| linux × x86_64/aarch64 | `libc.so.6` | `getpid` | `ioctl(TIOCGWINSZ)` | `getppid` | live `time`, `clock_gettime`, `uname`, `getuid`, `getgid`, `geteuid`, `getegid`, `sysconf(_SC_PAGESIZE)`, `sysconf(_SC_CLK_TCK)`, `sysconf(_SC_NPROCESSORS_ONLN)`, `getcwd`, `isatty(0)`, `open("/dev/null")` + `isatty` + `close`, and `access` success/failure dlcalls |
...
missing-path failure; `getenv("DISPLAY")`; honest `libX11`
