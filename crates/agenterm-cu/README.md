...
| `copy --window HANDLE` (no `--name`) | same innermost focused Text node `get-text --window` reads, then GetText published onto native CLIPBOARD; never XTest when `--window` is set. Chrome `GetTextField` after `focus --name`; Reasonix composer / con `Command` share the focused path. Proof is independent paste + `get-text --window` (no `--name`) equal to the seeded string |
