...
| `send-keys --window HANDLE` (no `--name`) | same innermost focused Text node `get-text --window` reads; prefer `DeviceEventListener.NotifyEvent` (`via=device-event`); plain typeable text falls back to AT-SPI Text write (`via=text`) when that interface is absent (Chrome `GetTextField` after `focus --name`; Reasonix composer `Message Reasonix…` after `focus --name`, eval-helper set-value). Never XTest when `--window` is set. Proof is independent `get-text --window` (no `--name`) |
