...
| `paste --window HANDLE` (no `--name`) | same innermost focused Text node `get-text --window` reads, then clipboard write via `agt_a11y_node_set_text`; never XTest when `--window` is set. Chrome `GetTextField` after `focus --name`; Reasonix composer `Message Reasonix…` after `focus --name` under `scripts/reasonix-desktop-a11y.sh` (eval-helper set-value, `via=text`); con `Command` after `focus --name` (`via=editable-text`). Proof is independent `get-text --window` (no `--name`) equal to the clipboard string |
