...
Global:
  ...
  worker over OpenSSH stdio (no new verb). Send-keys evidence: loopback sshd +
  second agenterm-con, focus Command, host send-keys --window then wait +
  get-text equals keys (focused path; never screenshot / --coords).
