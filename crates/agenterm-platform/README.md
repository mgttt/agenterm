...
| `process-reference` | owned stable process reference plus bounded or indefinite exact-object exit waits via HANDLE, pidfd, or kqueue; Windows target-process HANDLE delivery receipts and exact Job membership | target `libc` / minimal `windows-sys` |
