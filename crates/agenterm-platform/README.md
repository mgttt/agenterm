...
| `process-reference` | owned stable process reference plus bounded or indefinite exact-object exit waits via HANDLE, pidfd, or kqueue; Windows target-process HANDLE delivery receipts roll back until committed | target `libc` / minimal `windows-sys` |
