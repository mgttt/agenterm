...
5. 小步 `commit` + `push` 邮箱改动（可与代码同提交，或单独
   `docs(skills): mailbox …`）。
6. 需要主控立刻看见时，再尝试 API 推送；若 `409`，**以邮箱为准**，不要空转重试。

## 主控清单

1. 派活前写清：目标、禁止项、文件所有权、成功证据、是否允许合 `main`。
2. 分身 `IDLE` 才 API 推送；`RUNNING` 只写邮箱。
3. 用 MCP `list-cloud-agents` / `batch-fetch-details` 观测；大 transcript 用子代理。
4. 合流后更新邮箱「共享事实」里的 `main` SHA 与版本号。
...
| REST 推送 | 紧急改向、空闲时唤醒 | `409 agent_busy`；无回执线程 |
