...
impl Grid {
    ...
    pub fn col_wrap(&mut self, width: u16, wrap: bool) {
        ...
        if self.pos.col > self.size.cols - width {
            ...
            // prev_pos.row can be smaller than `scrolled` when a resize or a
            // one-line scroll region leaves the cursor on the region's top
            // row; saturate instead of aborting the process on underflow.
