...
("bigger than Q2's compiler *and* bigger than Q9's interpreter")。→ **未修改**(超出本次授权范围),
建议 Q10 owner 删去与 Q9 的比较,或按 Q9 口径补测 applier。
...
→ **未修改**,建议改写为 §2-U4 的正确形式。斜率结论本身不受影响。

**E7 — `primitives/RESULTS.md` ③ 表**:"+182 B .text … **≈+28% over Q5's 644 B**"(U6)。
→ **未修改**,建议删掉这个跨实验百分比,保留 "550→732, +33%"。
...
→ 综合前建议:Q2 补 `RESULTS.md`、把状态从 running 落定,否则整节判决的一角站在一个
"状态未决 + 无 RESULTS + 未执行的产物"上。
...
| # | 数字 | 不明之处 |
|---|---|---|
| N1 | Q2 "**162 shared / 139 x86-specific lines**"(以及 X 的 301 行) | **未声明是否去注释/空行**。全轨其余 LOC 都声明了"non-blank, non-comment"并被 Q9 交叉校准过;Q2 这一处**无法与 Q1 350 / Q5 307 并排**。 |
| N2 | Q2 "**minimal kernel baseline ≈ 2.93 KB (L) / ~4 KB (W)**" | **未写这个基线怎么测的**。数值与 Q0 的 kernel-only ~2738 + spawn 后 +208 = 2946 吻合,但 Q2 文档里没有这句话;它是"整个 stripped ELF"还是别的,**只能推,不能断**。而 in/out-kernel 判决(⑤ TCB)整个建在它上面。 |
| N3 | Q2 Windows 列 "**3003 (ELF flat)**" 与 Linux 列同值 | 表头是 "Windows (PE-aligned)",格内却写 "(ELF flat)" —— 即 Windows 侧**没有独立的 X 测量**,复用了 Linux 数。可读作口径说明,但表格形式上会被误读为"两 OS 各测一次都得 3003"。 |
...
| N7 | Q12 "~40 行 + ~30/34 字节/函数" | 明写是**估算,时间盒内未实现**。技术清单/执行层判决引用时未标"估算"。 |
...
**可比 12 组,不可比 10 组,已写下的不当比较 10 条(其中 3 条在「执行层判决」内,已修正;
5 条在技术清单/各 RESULTS 内,已点名未改;2 条为披露充分的小瑕疵),口径不明 8 处。**
