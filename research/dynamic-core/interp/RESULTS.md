...
| — 参考：Q0 四原语内核 | — | 568 | 只四原语 |
| — 参考：Q5 aarch64 内核 | — | 644 | 只四原语 |
...
- vs 内核：eval-core 1908 B ≈ 四原语内核 568 B 的 3.4×——同一量级（KB 级），因为它分发的是
  **完整 IR**（13 op/4 inst/4 term/6 intent），不是只有 4 个原语。

**→ KB 级、内核量级、且firmly 小于降级器。落判决树分支「解释可作一等退路」。**
...
**净结论**：把解释做成一等后端是**可行的优雅降级**——本体 ~2–3 KB（内核量级、降级器的 1/5）、
