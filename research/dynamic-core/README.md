...
| — where it stops | **L3b orchestration/control flow** (multi-call dataflow, SysV `fork`+branch — *no flat table has a branch*) and **I2 cross-ISA restructuring** (aarch64 has no `open`/`fork`: the syscall *set* changes shape, not just its number). Forcing either into data = inventing a call-sequencing bytecode = **the IDL slide** | `spawn_boundary()`: tablable-as-data **1**, needs-query-channel **2**, **irreducibly code 5**. Also unchecked by any schema: whether symbol index 1 *really is* `CreateFileA` — the naming *binding truth* stays trust | **[实测·结构推断] Q7 ⑤**; I2 folded in from **[实测·仅字节测量＋编码器验证] Q5**, analyzed not executed |
