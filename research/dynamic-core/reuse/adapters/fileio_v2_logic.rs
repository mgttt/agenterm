//! File-I/O adapter LOGIC, version 2 (TRUNCATED read) — deliberately INCOMPATIBLE
//! with v1 for criterion ②. Same FileCaps ABI, breaking behavioral change: read_file
//! never returns more than 8 bytes. A payload built against v1 (expecting the whole
//! file) would malfunction on v2 and vice-versa — so v1 and v2 are two incompatible
//! versions of "the same" capability. Content addressing must let both coexist and
//! bind each consumer to exactly the bytes it wants, with no anointing.

use crate::abi::Kernel;

const V2_MAX: usize = 8;
...
/// v2: read at most V2_MAX bytes (breaking change vs v1's full read).
pub extern "sysv64" fn read_file(
    ...
    let clamped = if cap > V2_MAX { V2_MAX } else { cap };
    os::read_file(unsafe { &*k }, path, buf, clamped)
}
