...
fn transpile_project_entry(entry: &str) -> (String, agenterm_rh::CdylibTranspileOutput) {
    ...
    let output =
        agenterm_rh::transpile_cdylib_with_project(&repo(), &source).unwrap_or_else(|error| {
            panic!("transpile {entry}: {error}");
        });
    (source, output)
}
...
fn assert_bundled_pack_builds(entry: &str) {
    ...
    let dir =
        agenterm::script_rh_cache::compile_source_to_cache_with_project(&source, Some(&repo()))
...
fn native_rh_task_without_entry_fails_manifest_qualification() {
    ...
    assert!(
        report.entries[0].error.as_deref().is_some_and(|error| {
...
fn candidate_aggregate_uses_native_bundled_execution() {
    ...
    assert!(
        output.rust.contains("pub fn build_manifest("),
        "{}",
        output.rust
    );
    assert!(
        output.rust.contains("pub fn validate_manifest("),
        "{}",
        output.rust
    );
    assert!(
...
fn candidate_verify_uses_native_bundled_execution() {
    ...
    assert_eq!(output.execution_mode.as_str(), "native", "{}", output.rust);
    assert!(
        output.rust.contains("pub fn verify_platform("),
        "{}",
        output.rust
    );
    assert!(
...
fn control_center_smoke_uses_native_bundled_pack() {
    ...
    assert!(!source.contains("[\"rh\", \"eval\"]"));
    assert_native_bundled_pack(
