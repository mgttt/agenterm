...
- Linux live libc probes + paired S-expr examples (pid/uid/gid/pgid/sid/pgrp,
  ...
  getdtablesize, getpagesize, `times`, …).
...
`clock_gettime` now prove caller-owned buffers through `ptr`. Keep Win/macOS as
placeholders. No C shim. Restore any process-global side effect before the test
ends (`umask` is the pattern).
