...
| Single executable staging | `scripts/rh/stage-artifact.rh` (+ `scripts/rh/lib/artifact_files.rh` `stage`/`stage_as`) | `scripts/stage-artifact.ps1` | `e087842` then native `.rh` cutover | public CLI black-box tests prove normal replacement, invalid-name rejection, and Windows running-image parking before replacement | PowerShell deleted; task entry is native `.rh`; archived Rhai under `scripts/archive/rhai/`; `stage-build` still uses Rhai lib until that task migrates |
