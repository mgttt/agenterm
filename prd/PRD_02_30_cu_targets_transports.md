...
- [x] `send-keys --window HANDLE` without `--name` targets the showing
  ...
  interface is absent (Chrome renderer entry) and the payload is plain
  typeable text, writes through AT-SPI `EditableText` / `Text` + toolkit
  set-value (`via=text`, same path as focused `send-text`). Never XTest /
  `input_inject::send_keys` when `--window` is set. Independent
  `get-text --window HANDLE` (no `--name`) must equal the typed string.
  Live: Chrome `GetTextField` after `focus --name` on the host AT-SPI bus
  (`AT_SPI_BUS` / `AT_SPI_BUS_ADDRESS`). Special chords without a key
  interface still typed-fail. Do not mark this leaf shipped on worker JSON.
