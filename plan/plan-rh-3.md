...
| **日期** | 2026-08-07 |
| **状态** | **进行中 M42f6**（cg28 + g13）：`lint` 已切 `.rh`。`supply-chain` 待 pack 修复；Wave3 余叶与 `candidate-*` 仍属后续 |
...
| M42f6g13 | INT-only `scripts/rh/lint.rh`；entry 切线 + 归档；回归 `lint_*`（`supply-chain` Native 转译已通但 pack cargo 仍因本地 string helper 类型失败，待 g14） | [x] |
| M42f6h | `release_candidate`/`qualification`/`package_qualified` lib 原生移植 + 剩余叶（`candidate-*`、`migration-audit`、Wave3：`supply-chain` pack 修复、`prd-alignment`/`prune`/`powershell`/`preflight`/`target-report`）；`target-report` 仍缺 `pop`/float。**codegen 28 后**：`array.push`/JSON 数组字面量已解；`path.parent` 与 Child/sleep 仍属后续；入口仍停 `.rhai` 直至叶达 Native | [ ] |
