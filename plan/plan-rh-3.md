...
| **日期** | 2026-08-07 |
| **状态** | **进行中 M42f6**（cg22）：M42f6g 已切；`performance-summary` 已切原生 `.rh`。**Wave2**：`rh-aot-smoke` → `preflight-benchmark` → `client-smoke` → `cross-platform-automation-audit` → `build-releases-index` →（M42f6h `release_candidate`）`candidate-*` → `migration-audit`；并行 codegen：`DirEntry.metadata.modified`/`keys`/`pop` 解锁 `target-report` |
