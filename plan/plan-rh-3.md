...
| M16 | rh-3b：赋值/复合赋值 + `while` 可变异计数 | [ ] |
| M17 | rh-3b：`try`/`catch` 子集 + 原生 throw 路径 | [ ] |
| M18 | rh-3c：`agenterm-rh check-many`（bounded manifest，对齐 lint.rhai） | [ ] |
| M19 | rh-3c：bootstrap / CI 默认构建 `agenterm-rh` 二进制 | [ ] |
| M20 | rh-3d：worker 路径 `Run`/`Eval` 黑盒 parity（rh_backend 扩展） | [ ] |
