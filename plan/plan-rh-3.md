...
**主控在 main 硬切换（2026-08-07）：** tip 已合入 `origin/main`（rev40–42 flips）。**rev43**：原生 JSON 字段/下标赋值（`rh_json_set_path*`）+ JSON `+=`；`path.len`/`a.b.len` while 条件；字符串拼接/`to_string` 赋值 RHS；subset TRACE。`build.rh` 达 HostEval；qualification 仍卡 `rhai::hash::fnv1a64` JSON value emit；orch/smoke 继续。并行背景任务继续。
