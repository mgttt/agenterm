...
| **日期** | 2026-08-07 |
| **状态** | **进行中 M42f5**：M42f5a–b2 宿主/codegen 面已齐（cg20：JSON builder/`stringify_pretty`/`split`）；下一步写 INT-only `build_metadata.rh` + `stage-build.rh` 切入口（M42f5c） |
...
| M42f5b3 | INT-only `scripts/rh/lib/build_metadata.rh`（`write` 返回 0；过程调用改 `command_stdout_file`/`command_status`） | [ ] |
