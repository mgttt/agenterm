...
| **日期** | 2026-08-06 |
| **状态** | **M42 推进中；M42d1–d5（至 import bundling）已完成，validate-artifact-manifest 无损迁移待做** |
...
| M42d | 无损迁移 `validate-artifact-manifest`（含字符串形参模块函数若仍缺则同批补齐）；不得用 substring 或任务专用宿主校验器替代脚本不变量 | [ ] |
