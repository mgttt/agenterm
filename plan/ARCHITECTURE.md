...
src/frontend/                产品 GUI 入口 + UI/UX 语义
  ...
  interaction.rs             focus navigation / wheel accumulation / wheel routing / scrollbar thumb drag / modal/focus state（语义 surface 存 FocusState + adapter focus_gate()，Win/Unix 共用）；raw-mouse arbitration 策略与 xterm mouse report 编码器（Unix embedded 与 Windows remote 共用）
