...
| L3 | `platform/mod.rs` 策略过肥（input/paths/control_center/runtime/test_fixtures/workspace 已拆 `policy/`；FrontendHost/能力映射等仍可再瘦身）+ `allow(dead_code)` | `policy/{input,paths,control_center,runtime,test_fixtures,workspace}` 全拆；禁新顶层 `is_windows_host` 蔓延 |
