...
src/frontend/                产品 GUI 入口 + UI/UX 语义
  ...
  interaction.rs             focus navigation / wheel accumulation / wheel routing / scrollbar thumb drag / modal/focus state（FocusState，Win/Unix 共用）；raw-mouse arbitration 策略（Unix embedded 消费，Windows remote 未接入）
