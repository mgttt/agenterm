...
| L3 | `platform/mod.rs` 策略过肥（input/control_center 已拆 `policy/`；paths/runtime/test_fixtures 仍在）+ `allow(dead_code)` | 拆 `policy/{input,paths,control_center,runtime,test_fixtures}`；禁新顶层 `is_windows_host` 蔓延 |
