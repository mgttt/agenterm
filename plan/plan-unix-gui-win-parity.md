...
├─ D3 [~] Win remote vs Unix embedded 共享交互管线（selection phase、focus 导航、wheel 累积/路由、scrollbar thumb drag 已收为 crate 共享；`FocusState` 存储语义 surface + adapter `focus_gate()` 共享 modal/focus 规则；raw-mouse 仲裁函数已建，Unix embedded 已接入，Windows remote 未接入）
