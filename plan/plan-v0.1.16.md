...
| 日期 | 决定 |
|------|------|
| 2026-08-12 | **当前唯一阻塞：新建 tab 的子命令参数在 Windows 上丢失（真回归，最小复现 30 秒）**。`fleet-smoke` 的 `fleet_tab_environment` / `fleet_codex_proxy_context` 红在这上面，windows lane 因此卡在 release-lane 一步。复现：<br>`agenterm cli --address X new-window -d -n probe -- cmd.exe /d /c "echo HELLO_MARKER"`<br>`agenterm cli --address X capture-pane -p -t probe`<br>期望打印 `HELLO_MARKER` 后退出，实际得到 **cmd 的交互式 banner + 提示符**，即子进程按裸 `cmd.exe` 启动、`/d /c ...` 全丢。`inspect` 也只记录 `"command": "cmd.exe"`。定位边界：CLI 侧解析**是对的**（`commands::tests::parses_new_window_options_and_child_command` 通过，`parse_new_command` 正确处理 `--`），所以丢失发生在「CLI 请求 → PTY spawn」之间，而 Windows PTY 后端刚在 `96b2c899`（自有 ConPTY 后端）、`a97af915`（统一线程启动 trampoline）被重写 —— 从那里查。<br>**排除的错误假设**（别重走）：① 不是抓取竞态 —— 加 `wait-pane --contains "AGENTERM_SESSION="` 等 30 秒仍然超时，那两个变量永远不出现；② `inspect` 的 `environment_names` 只列 `-e` 传入的变量，`["FLEET_ROLE"]` 是**正确**的，该半边断言本来就通过，别去改它。 |
