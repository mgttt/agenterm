...
| C-con-native-host | [~] | `crates/agenterm-con` 为独立 package，Windows 默认 User32/GDI，Linux/macOS 默认 portable；resolved normal graph 59 行且无 winit/softbuffer/Rhai/HTTP/TLS/script engine。Native 已接 IMM32 preedit/commit、候选框 client-anchor、pointer capture/loss、mouse-leave、checked DPI suggested-rect，并修复 GCS_CURSORPOS UTF-16→char。截图改为跨平台流式 stored-DEFLATE PNG，生产图移除 png/miniz/fdeflate/crc32fast，第三方 decoder 覆盖单块与跨 65,535 B 块互操作。73 con 单测、70 platform lib tests、control GUI 黑盒、terminal 黑盒 16 pass/2 known-gap ignored/0 fail；旧 unsupported runner 测试造成的超时孤儿/锁文件已改为 opened→Exit 生命周期证据。release PE 763,904 B。仅剩真实中文输入法人工验收 | 不把 Win32 泄漏进产品层；接受截图文件较大；不宣称已达 512 KiB |
