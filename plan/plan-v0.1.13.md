...
**0.1.13 目标**：拆 `policy/{input,paths,control_center,runtime,test_fixtures}`；`policy/input.rs` 已落地（2026-08-03），其余待拆；每表单测；禁止新的顶层 `is_windows_host()` 蔓延。
...
src/platform/
  ...
  policy/                       # 产品策略表（cfg-free）
    ...
    paths.rs  control_center.rs  runtime.rs  test_fixtures.rs  # 待拆
  adapters/
    windows/
