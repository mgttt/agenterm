...
- 新增并入回归约束：`tests/platform_boundary_regressions.rs` 对 `src/`（排除 `src/platform`）执行上述关键词扫描；未来若出现平台判定回流（含 `PlatformKind` 显式分支、`is_*_host`、`platform_kind`、`cfg` 编译期分支），测试会直接失败，作为非平台层行为边界失败前置。
