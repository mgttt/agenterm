# Plan tree: 薄 L1 冻底盘（chassis）+ 打包合成成品（不是再编译）
...
| **W1 合成器** | [`scripts/chassis-compose-product.py`](../scripts/chassis-compose-product.py) |
...
| 层 | 是什么 | 日常怎么动 |
|----|--------|------------|
| **Chassis-L1** | 每格一份薄 loader | 几乎不动；动则六格编译 |
| **Chassis-L2** | 应用看见的 ABI + 跨 OS 落实（表/包/cu 插件） | 换文件再打包 |
| **Chassis-L3** | 应用（class/jar 那一层） | 换包再打包 |

L3 只写能力名。L3 里出现 `libSystem` / `libc.so` / `kernel32` = 失败。  
L2 若还是半个 `agenterm.exe`，打包循环是假的。

命名用 **Chassis-L\***（底盘），避免和终端 **shell**（命令行）以及脚本 L1/L2/L3 混称。
...
- [`scripts/chassis-compose-product.py`](../scripts/chassis-compose-product.py) 只读已有 L1 字节 + L2/L3 文件树，写出确定性 tar。
- [`scripts/chassis-compose-product-test.py`](../scripts/chassis-compose-product-test.py) 用合成夹具跑两遍：
