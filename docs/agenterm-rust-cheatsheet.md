...
- Do not assume a native window or softbuffer preserves pixels across frames or resize unless its typed contract says so. Put retained pixels in a host-neutral product/core owner, force full raster on first frame, resize, invalid state, and unknown damage, then copy to transient host buffers until a platform damage contract exists.
