...
libagenterm). Get-extents evidence is loopback `sshd` plus a second
...
count. Never screenshot, `--coords`, or XTest. Missing / empty extents
typed-fail `a11y_extents_unavailable` on the remote worker the same as
local `current`. `get-caret` (3.28), `tree` (3.27), `focus` (3.26),
