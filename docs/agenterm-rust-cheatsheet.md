...
cuts 3.31 observe / 3.32 actuate). It does not invent verbs: the host
handshakes RFB (security type None / `x11vnc -nopw` only in this cut),
rewrites the abstract command to `target=current`, and runs a local
...
AT-SPI on that session — never RFB framebuffer OCR, screenshot, or
`--coords`. First write evidence is a **gate-owned** loopback x11vnc
(not the resident `:2` listener alone) plus a second `agenterm-con` on
a unique control socket and title: host `send-text --name Command`
plants a unique seed, then host independent `get-text --name Command`
equals that seed (`via=gettext`). Do not steal
