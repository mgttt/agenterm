...
- Raw Win32 callbacks may reenter synchronously through `SetWindowTextW`, `ShowWindow`, `SetWindowPos`, focus, capture, and related FFI. Never reconstruct a second `&mut HostState` from window userdata while an application callback or framebuffer borrow is live. Classify stateless messages before borrowing state; queue stateful native events and callback-issued native commands behind an explicit dispatch phase, copy pointer-backed message data immediately, bound the queues, and drain only after the application/frame borrow ends.
