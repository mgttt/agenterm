...
- Rust 1.97's `std` exposes `backtrace-trace-only` specifically for `-Zbuild-std`; paired with `panic-unwind` it can remove symbolization/demangling code while retaining catch semantics. Treat this as a toolchain boundary, not a casual `RUSTFLAGS` tweak: require pinned `rust-src`, explicit target, all-platform CI, reproducible sealed bytes, and measured PE sections before adoption. In the con experiment it reduced release-fast unwind from 849,920 B to 790,016 B, mostly through `.text`, but did not yet meet the 512 KiB budget.
