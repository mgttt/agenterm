...
6. [x] `HostResourceTable<T>` gives versioned native modules a bounded,
   domain- and generation-checked `i32` handle owner. Close, clear, cross-module
   use, slot reuse and complete generation exhaustion cannot make a token name
   the wrong object; a real cartridge proves create/read/close through standard
   imports.
