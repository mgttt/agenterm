...
cleanup() {
    if [ -n "${AGENTERM_BOOTSTRAP_RH_CLI:-}" ]; then
        rm -f -- "$AGENTERM_BOOTSTRAP_RH_CLI"
    fi
...
if [ -s "$RH_SOURCE" ]; then
    ...
    AGENTERM_BOOTSTRAP_RH_CLI="$BOOTSTRAP_DIR/agenterm-rh"
else
    AGENTERM_BOOTSTRAP_RH_CLI=""
fi
...
export AGENTERM_BOOTSTRAP_RH_CLI
...
AGENTERM_RHAI_COMPAT_CLI="$WORKER"
export AGENTERM_RHAI_COMPAT_CLI
TASK_CLI="$WORKER"
if [ -n "$AGENTERM_BOOTSTRAP_RH_CLI" ]; then
    TASK_CLI="$AGENTERM_BOOTSTRAP_RH_CLI"
fi

"$TASK_CLI" task run "$AGENTERM_BOOTSTRAP_TASK" \
